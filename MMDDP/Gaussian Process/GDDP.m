...
load initial.mat;
% x_norm = [x_norm;[pi,pi,0,0];[0,0,0,0];[pi,0,0,0];[0,pi,0,0]];
% u_norm = [u_norm;[0,0];[0,0];[0,0];[0,0]];
...
%         if du_max+dv_max< 0.1
        if du_max+dv_max<0.1 || (itr>30 && k>10) || (itr>k*3 && k<=10)
            break;
        end
    ...
    x0=[pi;pi;0;0];
        [~,x_data] = ode45(@(t,x) dynamics_data(t,x,u_pp), time, x0);
    x_norm=[x_norm;x_data(1:N,:)];
    u_norm=[u_norm;u'];
    save('nom_tra','x_norm','u_norm');
    % control sequence
figure(1);
